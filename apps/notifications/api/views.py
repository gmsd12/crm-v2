...
from apps.notifications.recipients import get_notification_watch_targets, replace_notification_watch_targets


class NotificationFilter(django_filters.FilterSet):
    class Meta:
        model = Notification
        fields = ["event_type", "status", "is_read", "lead"]
...
class NotificationViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def settings_me(self, request):
                ...
                items.append(
                    {
                        ...
                        "watch_targets": get_notification_watch_targets(watcher=request.user, event_type=event_type),
                        "resolved": {
        ...
        pref.save()
        try:
            watch_targets = replace_notification_watch_targets(
                watcher=request.user,
                event_type=event_type,
                watched_user_ids=serializer.validated_data.get("watched_user_ids")
                if "watched_user_ids" in serializer.validated_data
                else None,
                watched_roles=serializer.validated_data.get("watched_roles")
                if "watched_roles" in serializer.validated_data
                else None,
            )
    ...
    def stream(self, request):
        ...
        def event_stream_once():
            ...
            items = list(user_queryset.filter(id__gt=current_last_id).order_by("id")[:50])
            if items:
